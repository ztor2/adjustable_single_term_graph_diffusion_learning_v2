...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Variational Graph Auto-Encoder (VGAE) experiments with adjustable diffusion."
    )
    ...
    parser.add_argument("--hidden2", type=int, default=16, help="Latent embedding dimension.")
...
def _split_factors(test_ratio: float, val_ratio: float) -> Dict[str, float]:
...
def run_vgae_iteration(
    ...
    *,
    ...
    for run_idx in range(n_iter):
        ...
        model = GCN_VAE(feat_dim, hidden1, hidden2, dropout).to(device)
        ...
        for epoch in range(epochs):
            model.train()
            optimizer.zero_grad()
            recon_adj, mu, logvar = model(feat_tensor, adj_norm)
            loss = loss_function(
                preds=recon_adj,
                labels=adj_label,
                mu=mu,
            ...
            if not quiet and (epoch + 1) % max(log_interval, 1) == 0:
                ...
                roc_curr, ap_curr = get_roc_score_vgae(emb, adj_orig, val_edges, val_edges_false)
                print(
        ...
        with torch.no_grad():
            emb = mu.detach().cpu().numpy()
        roc_score, ap_score = get_roc_score_vgae(emb, adj_orig, test_edges, test_edges_false)
